        ...
        with httpx.Client(timeout=httpx.Timeout(15.0)) as client:
            while _time.monotonic() - start < max_wait:
                ...
                if poll_resp.status_code == 200:
                    ...
                    # OpenAI may rate-limit polling before the user approves.
                    # Soft retry: back off (honoring Retry-After) and poll again.
                    retry_after = _parse_retry_after_seconds(
                        getattr(poll_resp, "headers", None)
                    )
                    delay = retry_after if retry_after is not None else poll_interval * 2
                    _time.sleep(max(poll_interval, min(int(delay), 60)))
                    continue
                else:
                    raise AuthError(
